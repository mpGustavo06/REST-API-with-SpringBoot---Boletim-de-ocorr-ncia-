...
@Provider
public class ConteinersInterceptor implements ContainerResponseFilter {
   private final Integer corsPreflightMaxAgeInSeconds = 1800;

   public void filter(ContainerRequestContext request, ContainerResponseContext response) throws IOException {
      response.getHeaders().add("Access-Conrol-Allow-Origin", request.getHeaderString("origin"));
      response.getHeaders().add("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE");
      response.getHeaders().add("Access-Control-Allow-Credentials", "true");
      
      List<String> allowedHeaders = request.getHeaders().get("Access-Control-Request-Headers");
